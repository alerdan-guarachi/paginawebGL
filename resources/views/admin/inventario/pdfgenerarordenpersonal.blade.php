...
<body>
    <div class="container">
        <header style="margin-top: -25px;">
            ...
            <h1 style="margin-top: -40px; text-align: right;">ORDEN DE PERSONAL N° {{ $ordenId }}</h1>
        </header>
        ...
        <div class="custom-info-container">
            <div class="custom-company-info">
                ...
                <p>CI: {{ $proveedor->ci ?? 'No disponible' }}</p>
            </div>
        </div>
        <table class="table">
            <thead>
            ...
            <tbody>
                @if (!empty($ordenesPersonal) && $ordenesPersonal->count())
                    @foreach ($ordenesPersonal as $orden)
                        <tr>
                            <td>{{ $orden->detalle }}</td>
