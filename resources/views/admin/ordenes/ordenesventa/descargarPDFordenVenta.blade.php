    ...
    <div class="container">
        ...
        <div class="info-container"
            ...
            <div class="company-info" style="flex: 1; margin-right: 20px;">
                <p style="margin: 0 0 5px 0;">NIT: {{ $empresaDatos['nit'] }}</p>
                <p style="margin: 0 0 5px 0;">SANTA CRUZ: {{ $empresaDatos['direccion_santa_cruz'] }}</p>
                <p style="margin: 0 0 5px 0;">COCHABAMBA: {{ $empresaDatos['direccion_cochabamba'] }}</p>
                <p style="margin: 0;">TELÉFONO: {{ $empresaDatos['telefonos'] }}</p>
            </div>
            ...
            <div class="header-right" style="text-align: right; flex: 1;">
                ...
                <p style="margin: 0; padding-bottom: 0;">MODALIDAD DE PAGO:<span
                        style="border: 0.5px solid lightgray; border-bottom: 2px solid lightgray; padding: 2px; display: inline-block; min-width: 100px; margin-left: 20px; vertical-align: bottom;">{{ $modalidadpago ?? 'No especificado' }}</span>
                </p>
                ...
                <p style="margin: 0; padding-bottom: 0;">FECHA DE PAGO:<span
                        style="border: 0.5px solid lightgray; border-bottom: 2px solid lightgray; padding: 2px; display: inline-block; min-width: 100px; margin-left: 20px; vertical-align: bottom;">{{ $fechapago ?? \Carbon\Carbon::now()->toDateString() }}</span>
                </p>
            </div>
        </div>
        ...
        <div class="custom-info-container">
            ...
            <div class="custom-header-right">
                ...
                <p>ENTIDAD FINANCIERA: {{ $banco ?? 'No especificado' }}</p>
                <p>NRO DE CUENTA: {{ $numerocuenta ?? 'No especificado' }}</p>
                <p>TITULAR: {{ $titularcuenta ?? 'No especificado' }}</p>
            </div>
        </div>
            ...
            <tbody>
                @foreach ($bateriasubclientes as $bateriasubcliente)
                    <tr>
                        ...
                        <td>{{ $salida ?? 'Banco' }}</td>
                        <td>{{ $destino ?? 'Banco' }}</td>
                        <td>{{ $formapago ?? 'Transferencia bancaria' }}</td>
                        {{-- <td>22/10/2024</td> --}}
        ...
        <div class="totals-container"
            ...
            <div class="company-info" style="width: 42.7%; padding: 5px; border: 2px solid #696969;">
                ...
                <div style="height: 80px; background-color: white;">{{ $observaciones ?? '' }}</div>
            </div>
            ...
            <div class="totals" style="text-align: right; flex-grow: 1; margin-left: 20px;" style="margin-top: -300px;">
                ...
                <p style="margin: 0; padding-bottom: 0;"><strong>TOTAL:</strong><span
                        style="border: 0.5px solid lightgray; border-bottom: 2px solid lightgray; padding: 2px; display: inline-block; min-width: 100px; margin-left: 20px; vertical-align: bottom;">{{ $total - ($descuento ?? 0) }}</span>
                </p>
            </div>
        </div>
