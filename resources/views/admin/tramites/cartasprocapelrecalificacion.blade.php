...
<a class="btn btn-sm float-right btn-regresar" href="{{ route('admin.tramites.procapelrecalificacion', $cliente->id) }}">REGRESAR</a>
<h5>MISIVAS DE APELACIÓN DE RECALIFICACIÓN DE:</h5>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarsolicitud', $cliente) }}" method="GET" enctype="multipart/form-data" id="formSolicitud">
                                        ...
                                        <input type="text" class="form-control" id="tramite" name="tramite" value="APELACIÓN DE RECALIFICACIÓN" hidden>
                                        ...
                                        <div class="row">
                                                                                                            ...
                                                                                                            <td class="text-center align-middle">
                                                                                                                @php
                                                                                                                    ...
                                                                                                                    $ruta = match(true) {
                                                                                                                        ...
                                                                                                                        in_array($tipoarea, ['PROGRAMACIONES', 'INFORMES ADICIONALES']) => "tramitesclientesita/{$cliente->id}/APELACIÓN DE RECALIFICACIÓN/INFORMES/{$doc->document}",
                                                                                                                        $tipoarea === 'RSRD'=> "tramitesclientesita/{$cliente->id}/APELACIÓN DE RECALIFICACIÓN/RSRD - ADJUNTO DOCUMENTACIÓN MÉDICA/{$doc->document}",
                                            ...
                                            <div class="col-lg-12" id="tablaadjuntosContainer" style="display: none;">
                                                ...
                                                {{-- <input type="text" class="form-control" id="tipocartareclamo" name="tipocartareclamo" value="PENSIÓN DE APELACIÓN DE RECALIFICACIÓN" hidden> --}}
                                            </div>
                                            <script>
                                                $(document).ready(function() {
                                                    let filaCount2 = 1;
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generaradjuntoyrespuesta', $cliente) }}" method="GET" enctype="multipart/form-data" id="formAdjunto">
                                        ...
                                        <input type="text" class="form-control" id="tramite2" name="tramite2" value="APELACIÓN DE RECALIFICACIÓN" hidden>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarcartareclamo', $cliente) }}" method="GET" enctype="multipart/form-data" id="formCarta">
                                        ...
                                        <input type="text" class="form-control" id="tramite" name="tramite3" value="APELACIÓN DE RECALIFICACIÓN" hidden>
                                        <input type="date" class="form-control" id="fechasubida3" name="fechasubida3" value="{{ \Carbon\Carbon::now()->toDateString() }}" hidden>
                                        @csrf
                                        <div class="row">
                                            @php
                                                $documento1 = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'INGRESO DE TRÁMITE')->where('subprocedimiento', 'RECEPCIÓN DE TRÁMITE')->first();
                                                $documento5 = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'FIRMA EAP')->where('subprocedimiento', 'ESTADO DE AHORRO PREVISIONAL')->first();
                                                $fechaingresoyeap = $documento1 && $documento5;

                                                $primeracartasit = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA SIT')->first();
                                                $segundacartasit = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA SIT')->first();
                                                $terceracartasit = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA SIT')->first();
                                                $primeracartareclamo = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA DE RECLAMO')->first();
                                                $segundacartareclamo = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA DE RECLAMO')->first();
                                                $terceracartareclamo = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA DE RECLAMO')->first();
                                                $cartareclamoaps = $cliente->tramites()->where('tramite', 'APELACIÓN DE RECALIFICACIÓN')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'CARTA DE RECLAMO APS')->first();
                                            @endphp
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarlibre', $cliente) }}" method="GET" enctype="multipart/form-data" id="formLibre">
                                        ...
                                        <input type="text" class="form-control" id="tramite4" name="tramite4" value="APELACIÓN DE RECALIFICACIÓN" hidden>
