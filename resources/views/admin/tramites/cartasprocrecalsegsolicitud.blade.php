...
<a class="btn btn-sm float-right btn-regresar" href="{{ route('admin.tramites.procrecalsegsolicitud', $cliente->id) }}">REGRESAR</a>
<h5>MISIVAS DE RECALIFICACIÓN SEGUNDA SOLICITUD DE:</h5>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarsolicitud', $cliente) }}" method="GET" enctype="multipart/form-data" id="formSolicitud">
                                        ...
                                        <input type="text" class="form-control" id="tramite" name="tramite" value="RECALIFICACIÓN SEGUNDA SOLICITUD" hidden>
                                        ...
                                        <div class="row">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="form-group col-lg-3">
                                                        ...
                                                        {!! Form::select('tipo_pdf', [
                                                            ...
                                                            'RECALIFICACIÓN SEGUNDA SOLICITUD DE DICTAMEN' => 'RECALIFICACIÓN SEGUNDA SOLICITUD DE DICTAMEN',
                                                                                                            ...
                                                                                                            <td class="text-center align-middle">
                                                                                                                @php
                                                                                                                    ...
                                                                                                                    $ruta = match(true) {
                                                                                                                        ...
                                                                                                                        in_array($tipoarea, ['PROGRAMACIONES', 'INFORMES ADICIONALES']) => "tramitesclientesita/{$cliente->id}/RECALIFICACIÓN SEGUNDA SOLICITUD/INFORMES/{$doc->document}",
                                            ...
                                            <div class="col-lg-12" id="tablaadjuntosContainer" style="display: none;">
                                                ...
                                                {{-- <input type="text" class="form-control" id="tipocartareclamo" name="tipocartareclamo" value="PENSIÓN DE RECALIFICACIÓN SEGUNDA SOLICITUD" hidden> --}}
                                            </div>
                                            <script>
                                                $(document).ready(function() {
                                                    let filaCount2 = 1;
                                        ...
                                        <script>
                                            $(document).ready(function() {
                                                ...
                                                const apoderadoOpciones = [
                                                    ...
                                                    'RECALIFICACIÓN SEGUNDA SOLICITUD DE DICTAMEN'
                                                ];
                                            ...
                                            $(document).ready(function() {
                                                $('#tipoPdfSelect2').on('change', function() {
                                                    ...
                                                    if (selectedValue === 'ACTUALIZACIÓN DE DATOS') {
                                                        ...
                                                        $('#nombremedicoContainer').show();
                                                    } else if (selectedValue === 'RECALIFICACIÓN SEGUNDA SOLICITUD DE DICTAMEN') { 
                                                        $('#fechatramiteContainer').show();  
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generaradjuntoyrespuesta', $cliente) }}" method="GET" enctype="multipart/form-data" id="formAdjunto">
                                        ...
                                        <input type="text" class="form-control" id="tramite2" name="tramite2" value="RECALIFICACIÓN SEGUNDA SOLICITUD" hidden>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarcartareclamo', $cliente) }}" method="GET" enctype="multipart/form-data" id="formCarta">
                                        ...
                                        <input type="text" class="form-control" id="tramite" name="tramite3" value="RECALIFICACIÓN SEGUNDA SOLICITUD" hidden>
                                        <input type="date" class="form-control" id="fechasubida3" name="fechasubida3" value="{{ \Carbon\Carbon::now()->toDateString() }}" hidden>
                                        @csrf
                                        <div class="row">
                                            @php
                                                $documento1 = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'INGRESO DE TRÁMITE')->where('subprocedimiento', 'RECEPCIÓN DE TRÁMITE')->first();
                                                $documento5 = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'FIRMA EAP')->where('subprocedimiento', 'ESTADO DE AHORRO PREVISIONAL')->first();
                                                $fechaingresoyeap = $documento1 && $documento5;

                                                $primeracartasit = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA SIT')->first();
                                                $segundacartasit = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA SIT')->first();
                                                $terceracartasit = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA SIT')->first();
                                                $primeracartareclamo = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA DE RECLAMO')->first();
                                                $segundacartareclamo = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA DE RECLAMO')->first();
                                                $terceracartareclamo = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA DE RECLAMO')->first();
                                                $cartareclamoaps = $cliente->tramites()->where('tramite', 'RECALIFICACIÓN SEGUNDA SOLICITUD')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'CARTA DE RECLAMO APS')->first();
                                            @endphp
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="{{ route('admin.tramites.generarlibre', $cliente) }}" method="GET" enctype="multipart/form-data" id="formLibre">
                                        ...
                                        <input type="text" class="form-control" id="tramite4" name="tramite4" value="RECALIFICACIÓN SEGUNDA SOLICITUD" hidden>
