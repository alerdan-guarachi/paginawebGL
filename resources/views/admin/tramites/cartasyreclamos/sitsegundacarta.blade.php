...
<body> 
    <main>
        ...
        <div class="tipo5"><strong>REF.- SEGUNDA SOLICITUD DE INFORMACIÓN DE TRÁMITE DE</strong></div>
        <div class="tipo5">
            @php
        ...
        @if (!Str::startsWith($nombretramite, 'APELACIÓN'))
            <div class="tipo6">
                ...
                @if($fecha1sit)
                    ...
                    <strong>{!! $fecha1sit ?? '<span class="textoedita">FECHA 1ER. CARTA SIT</span>' !!}</strong> 
                    se presentó una carta solicitando información sobre el avance del trámite{!! $fecharesp1sit ? ',' : '.' !!}
                    @if($fecharesp1sit)
                        a la cual se nos dio una respuesta por parte de su institución publica con 
                        <strong>NOTA CITE {!! $cite1sit ?? '<span class="textoedita">NOTA CITE 1ER. CARTA SIT</span>' !!}</strong> con fecha 
        ...
        @if ($nombretramite === 'APELACIÓN' || Str::startsWith($nombretramite, 'APELACIÓN'))
            <div class="tipo6">
                ...
                por <strong>{!! $motivoorigendictamen ?? '<span class="textoedita">MOTIVO ORIGEN DICTAMEN</span>' !!}</strong>. Posteriormente en fecha 
                <strong>{!! $fecha1sit ?? '<span class="textoedita">FECHA 1ER. CARTA SIT</span>' !!}</strong> 
                se presentó una carta solicitando información sobre el avance del trámite{!! $fecharesp1sit ? ',' : '.' !!}
                @if($fecharesp1sit)
                    a la cual se nos dio una respuesta por parte de su institución publica con 
