...
<body> 
    <main>
        ...
        <div class="tipo5"><strong>REF.- REITERACIÓN A CARTAS DE RECLAMO DE TRÁMITE DE</strong></div>
        <div class="tipo5">
            @php
        ...
        <div class="tipo6">
            ...
            @else
                <strong>{{ $nombretramite }}</strong>
            @endif
            se informa que dentro su Entidad APS se realizó lo siguiente:
        </div>
        ...
        <div class="tipo6" style="margin-top: -15px;">
            <ul style="list-style-type: disc; padding-left: 20px;">
                @if($fecha1reclamoaps)
                    <li>
                        En fecha <strong>{!! $fecha1reclamoaps ?? '<span class="textoedita">FECHA CARTA</span>' !!}</strong>, se presentó una Carta de Reclamo.
                        @if($fecharesp1reclamoaps)
                            Esta fue respondida mediante
                            <strong>NOTA CITE {!! $cite1reclamoaps ?? '<span class="textoedita">NOTA CITE</span>' !!}</strong> con fecha 
                            <strong>{!! $fechacite1reclamoaps ?? '<span class="textoedita">FECHA NOTA CITE</span>' !!}</strong>, la cual fue recogida el 
                            <strong>{!! $fecharesp1reclamoaps ?? '<span class="textoedita">FECHA RESPUESTA</span>' !!}</strong>, 
                            en donde se indicó: {!! $texto1reclamoaps ?? '<span class="textoedita">TEXTO COMP.</span>' !!}
                        @endif
                    </li>
                    @if($fechaform1reclamoaps)
                        <li>
                            En fecha <strong>{{$fechaform1reclamoaps}}</strong> 
                            se presentó dentro Buzón de Reclamos y Sugerencias APS el Formulario de Reclamo <strong>Nº {{ $nroform1reclamoaps }}.</strong>
                        </li>
                    @endif
                @endif
                @if($fecha2reclamoaps)
                    <li>
                        En fecha <strong>{!! $fecha2reclamoaps ?? '<span class="textoedita">FECHA CARTA</span>' !!}</strong>, se presentó una Segunda Carta de Reclamo.
                        @if($fecharesp2reclamoaps)
                            Esta fue respondida mediante
                            <strong>NOTA CITE {!! $cite2reclamoaps ?? '<span class="textoedita">NOTA CITE</span>' !!}</strong> con fecha 
                            <strong>{!! $fechacite2reclamoaps ?? '<span class="textoedita">FECHA NOTA CITE</span>' !!}</strong>, la cual fue recogida el 
                            <strong>{!! $fecharesp2reclamoaps ?? '<span class="textoedita">FECHA RESPUESTA</span>' !!}</strong>, 
                            en donde se indicó: {!! $texto2reclamoaps ?? '<span class="textoedita">TEXTO COMP.</span>' !!}
                        @endif
                    </li>
                    @if($fechaform2reclamoaps)
                        <li>
                            En fecha <strong>{{$fechaform2reclamoaps}}</strong> 
                            se presentó dentro Buzón de Reclamos y Sugerencias APS el Formulario de Reclamo <strong>Nº {{ $nroform2reclamoaps }}.</strong>
                        </li>
                    @endif
                @endif
                @if($fecha3reclamoaps)
                    <li>
                        En fecha <strong>{!! $fecha3reclamoaps ?? '<span class="textoedita">FECHA CARTA</span>' !!}</strong>, se presentó una Tercera Carta de Reclamo.
                        @if($fecharesp3reclamoaps)
                            Esta fue respondida mediante
                            <strong>NOTA CITE {!! $cite3reclamoaps ?? '<span class="textoedita">NOTA CITE</span>' !!}</strong> con fecha 
                            <strong>{!! $fechacite3reclamoaps ?? '<span class="textoedita">FECHA NOTA CITE</span>' !!}</strong>, la cual fue recogida el 
                            <strong>{!! $fecharesp3reclamoaps ?? '<span class="textoedita">FECHA RESPUESTA</span>' !!}</strong>, 
                            en donde se indicó: {!! $texto3reclamoaps ?? '<span class="textoedita">TEXTO COMP.</span>' !!}
                        @endif
                    </li>
                    @if($fechaform3reclamoaps)
                        <li>
                            En fecha <strong>{{$fechaform3reclamoaps}}</strong> 
                            se presentó dentro Buzón de Reclamos y Sugerencias APS el Formulario de Reclamo <strong>Nº {{ $nroform3reclamoaps }}.</strong>
                        </li>
                    @endif
                @endif
            </ul>
