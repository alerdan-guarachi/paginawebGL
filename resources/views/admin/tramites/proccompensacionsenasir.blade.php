...
<h5>PROCEDIMIENTO DE TRÁMITE DE COMPENZACIÓN SENASIR DE:</h5>
...
<div class="card">
    ...
    <div class="card-header">
        <ul class="nav nav-tabs card-header-tabs" id="myTabs">
            ...
            @php
                $documento5 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'ESTADO DE AHORRO PREVISIONAL')->where('estadocomunicado', 'COMUNICADO')->first();
            @endphp
            @if (!$documento5)
            <li class="nav-item">
            ...
            <div class="tab-pane fade show active" id="tab-content-1" role="tabpanel" aria-labelledby="tab-1">
                <div class="row">
                    ...
                    <div class="col-12 col-md-12 mb-3">
                        ...
                        <br>
                        @php
                            $documento1 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'RECEPCIÓN DE TRÁMITE')->first();
                            $documento2 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'INCLUSIÓN DE PODER')->first();
                        @endphp
                    ...
                    {{-- @php
                        $documento1 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'RECEPCION DE TRÁMITE')->where('estadocomunicado', 'COMUNICADO')->first();
                        $documento2 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'INCLUSION DE PODER')->where('estadocomunicado', 'COMUNICADO')->first();
                    @endphp
                    <div class="col-12 col-md-4 mb-3">
                        ...
                        <br>
                        @php
                            $documento3 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'VALIDACIÓN DE PODER')->first();
                        @endphp
                        <div class="text-center">
                            @if (!$documento3)
                                <span class="mb-0 checkamarillo">
                    ...
                    {{-- FIRMA EAP --}}
                    {{-- @php
                        $documento3 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'VALIDACIÓN DE PODER')->where('estadocomunicado', 'COMUNICADO')->first();
                    @endphp
                    <div class="col-12 col-md-4 mb-3">
                        <button type="button" class="btn btn-custom btn-block text-center" data-toggle="modal" data-target="#modalFirmaEAP" @if (!$documento3) disabled @endif>
                            <div class="d-flex flex-column align-items-center justify-content-center">
                                ...
                                @php
                                    $documento5 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'ESTADO DE AHORRO PREVISIONAL')->first();
                                @endphp
                                @if (!$documento5)
                                    @if ($documento3)
                                        @php
            ...
            <div class="tab-pane fade" id="tab-content-2" role="tabpanel" aria-labelledby="tab-2">
                <div class="row">
                    ...
                    <div class="col-12 col-md-12 mb-3">
                        ...
                        <br>
                        @php
                            $documento6 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'NOTIFICACIÓN DE GESTORA')->first();
                            $documento7 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'SOLICITUD DE CERTIFICADO DE OBITO LEGALIZADO')->first();
                            $documento8 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'RESPUESTA A NOTA DE GESTORA')->first();
                            $documento9 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'ADJUNTO DE DECLARACIÓN DE HEREDEROS')->first();
                        @endphp
                        <div class="text-center">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{ route('admin.tramites.guardartramitesclienteita', $cliente) }}" method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="container">
                                                ...
                                                @php
                                                    $documento6 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'NOTIFICACIÓN DE SENASIR')->first();
                                                @endphp
                                                <div class="row mb-3 align-items-center {{ !$documento6 ? 'no-documento' : '' }}">
                                                    <div class="col-md-4 text-center">
                                                        <p class="mb-0">NOTIFICACIÓN DE SENASIR</p>
                                                        @if (!$documento6)
                                                        <input type="text" class="form-control" id="tramite1" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
                                                        ...
                                                        <input type="text" class="form-control" id="subprocedimiento1" name="subprocedimiento[]" value="NOTIFICACIÓN DE SENASIR" hidden>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-4 text-center">
                                                        @php
                                                            $documento6 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS')->where('subprocedimiento', 'NOTIFICACIÓN DE GESTORA')->first();
                                                        @endphp
                                                        @if ($documento6)
                                                ...
                                                @php
                                                    $documento7 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO AL EMMPLEADOR')->first();
                                                @endphp
                                                <div class="row mb-3 align-items-center {{ !$documento7 ? 'no-documento' : '' }}">
                                                    <div class="col-md-4 text-center">
                                                        <p class="mb-0">SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO AL EMMPLEADOR</p>
                                                        @if (!$documento7)
                                                        <input type="text" class="form-control" id="tramite1" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
                                                        <input type="text" class="form-control" id="nivelprocedimiento1" name="nivelprocedimiento[]" value="NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS" hidden>
                                                        <input type="text" class="form-control" id="subprocedimiento1" name="subprocedimiento[]" value="SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO AL EMMPLEADOR" hidden>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-4 text-center">
                                                        @php
                                                            $documento7 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS')->where('subprocedimiento', 'SOLICITUD DE CERTIFICADO DE OBITO LEGALIZADO')->first();
                                                        @endphp
                                                        @if ($documento7)
                                                ...
                                                @php
                                                    $documento8 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO A LA CAJA')->first();
                                                @endphp
                                                <div class="row mb-3 align-items-center {{ !$documento8 ? 'no-documento' : '' }}">
                                                    <div class="col-md-4 text-center">
                                                        <p class="mb-0">SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO A LA CAJA</p>
                                                        @if (!$documento8)
                                                        <input type="text" class="form-control" id="tramite1" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
                                                        <input type="text" class="form-control" id="nivelprocedimiento1" name="nivelprocedimiento[]" value="NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS" hidden>
                                                        <input type="text" class="form-control" id="subprocedimiento1" name="subprocedimiento[]" value="SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO A LA CAJA" hidden>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-4 text-center">
                                                        @php
                                                            $documento8 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS')->where('subprocedimiento', 'SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO A LA CAJA')->first();
                                                        @endphp
                                                        @if ($documento8)
                                                ...
                                                @php
                                                    $documento9 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'ADJUNTO Y RESPUESTA A SOLICITUD DEL SENASIR')->first();
                                                @endphp
                                                <div class="row mb-3 align-items-center {{ !$documento9 ? 'no-documento' : '' }}">
                                                    <div class="col-md-4 text-center">
                                                        <p class="mb-0">ADJUNTO Y RESPUESTA A SOLICITUD DEL SENASIR</p>
                                                        @if (!$documento9)
                                                        <input type="text" class="form-control" id="tramite1" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
                                                        <input type="text" class="form-control" id="nivelprocedimiento1" name="nivelprocedimiento[]" value="NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS" hidden>
                                                        <input type="text" class="form-control" id="subprocedimiento1" name="subprocedimiento[]" value="ADJUNTO Y RESPUESTA A SOLICITUD DEL SENASIR" hidden>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-4 text-center">
                                                        @php
                                                            $documento9 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'NOTIFICACIONES, SOLICITUDES, ADJUNTOS Y RESPUESTAS')->where('subprocedimiento', 'ADJUNTO Y RESPUESTA A SOLICITUD DEL SENASIR')->first();
                                                        @endphp
                                                        @if ($documento9)
                                            ...
                                            @php
                                                $documento6 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'NOTIFICACIÓN DE GESTORA')->first();
                                                $documento7 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO AL EMPLEADOR')->first();
                                                $documento8 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'SOLICITUD DE CERTIFICACIÓN DE APORTES Y/O BOLETAS DE PAGO A LA CAJA')->first();
                                                $documento9 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'ADJUNTO Y RESPUESTA A SOLICITUD DEL SENASIR')->first();
                                            @endphp
                                            @if (!$documento6 || !$documento7 || !$documento8 || !$documento9)
                    ...
                    <div class="col-12 col-md-12 mb-3">
                        <button type="button" class="btn btn-custom btn-block text-center" data-toggle="modal" data-target="#modalContrato">
                            <div class="d-flex flex-column align-items-center justify-content-center">
                                ...
                                <span class="h6 mb-0 btn-block text-center">COMPENZACIÓN DE COTIZACIONES</span>
                            </div>
                        </button>
                        <br>
                        @php
                            $documento11 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'FIRMA DE CONTRATO')->first();
                            $documento13 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'NOTA DE RECHAZO DE TRÁMITE')->first();
                        @endphp
                        <div class="text-center">
                    ...
                    <div class="container">
                        ...
                        <div class="row mb-3 align-items-center {{ !$documento2 ? 'no-documento' : '' }}">
                            <div class="col-md-4 text-center">
                                ...
                                <input type="text" class="form-control" id="tramite2" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
            ...
            <div class="modal-body">
                <form action="{{ route('admin.tramites.guardartramitesclienteita', $cliente) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="container">
                        ...
                        @php
                        $documento4 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'RECHAZO DE PODER')->first();
                        @endphp
                    ...
                    @php
                        $documento3 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'VALIDACION DE PODER')->first();
                        $documento4 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('subprocedimiento', 'RECHAZO DE PODER')->first();
                    @endphp
                    @if (!$documento3)
                        <button type="submit" class="btn btn-subirarchivos d-block mx-auto mb-3" target="_blank" style="width: fit-content;">SUBIR ARCHIVOS</button>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('admin.tramites.generarcartareclamo', $cliente) }}" method="GET" enctype="multipart/form-data">
                    ...
                    {!! Form::hidden('clienteitanombre', $cliente->nombrecompleto) !!}
                    {!! Form::hidden('apoderado', auth()->user()->name) !!}
                    <input type="text" class="form-control" id="tramite" name="tramite" value="COMPENZACIÓN SENASIR" hidden>
                    <input type="date" class="form-control" id="fechasubida" name="fechasubida" value="{{ \Carbon\Carbon::now()->toDateString() }}" hidden>
                    @csrf
                    <div class="row">
                        @php
                            $documento1 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'INGRESO DE TRÁMITE')->where('subprocedimiento', 'RECEPCIÓN DE TRÁMITE')->first();
                            $documento5 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'FIRMA EAP')->where('subprocedimiento', 'ESTADO DE AHORRO PREVISIONAL')->first();
                            $fechaingresoyeap = $documento1 && $documento5;

                            $primeracartasit = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA SIT')->first();
                            $segundacartasit = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA SIT')->first();
                            $terceracartasit = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA SIT')->first();
                            $primeracartareclamo = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'PRIMERA CARTA DE RECLAMO')->first();
                            $segundacartareclamo = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'SEGUNDA CARTA DE RECLAMO')->first();
                            $terceracartareclamo = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'TERCERA CARTA DE RECLAMO')->first();
                            $cartareclamoaps = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CARTAS / RECLAMOS')->where('subprocedimiento', 'CARTA DE RECLAMO APS')->first();
                        @endphp
                        ...
                        <input type="text" class="form-control" id="tipocartareclamo" name="tipocartareclamo" value="COMPENZACIÓN SENASIR" hidden>
...
<!-- COMPENZACION DE COTIZACIONES -->
<div class="modal fade" id="modalContrato" tabindex="-1" role="dialog" aria-labelledby="modalContratoLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title titulomodal" id="modalContratoLabel">COMPENZACIÓN DE COTIZACIONES</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.tramites.guardartramitesclienteita', $cliente) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="container">
                        ...
                        @php
                            $documento11 = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'CONTRATO')->where('subprocedimiento', 'FIRMA DE CONTRATO')->first();
                        @endphp
                        <div class="row mb-3 align-items-center {{ !$documento11 ? 'no-documento' : '' }}" style="margin-top: -10px;">
                            <div class="col-md-3 text-center">
                                <p class="mb-0">FORMULARIO DE CÁLCULO AUTOMÁTICO SENASIR</p>
                                @if (!$documento11)
                                <input type="text" class="form-control" id="tramite2" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
                        ...
                        <div class="row mb-3 align-items-center {{ !$documento13 ? 'no-documento' : '' }}">
                            <div class="col-md-4 text-center">
                                <p class="mb-0">FORMULARIO DE CÁLCULO MANUAL SENASIR</p>
                                @if (!$documento13)
                                <input type="text" class="form-control" id="tramite3" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
            ...
            <div class="modal-body">
                <form id="seguimientoForm" action="{{ route('admin.tramites.guardartramitesclienteitaseguimiento', $cliente) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="container">
                        ...
                        @php
                            $seguimientos = $cliente->tramites()->where('tramite', 'COMPENZACIÓN SENASIR')->where('nivelprocedimiento', 'SEGUIMIENTO')->get();
                        @endphp
                        ...
                        <div class="row mb-3 align-items-center no-documento">
                            <div class="col-md-3 text-center" hidden>
                                <input type="text" class="form-control" name="tramite[]" value="COMPENZACIÓN SENASIR" hidden>
