...
use App\Models\DictamenAuditoria;
...
class AsociadoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $asociados = Asociado::whereNotIn('id', [3,2, 6])
                          ->where('asociado', 'LIKE', "%$nombreasociado%")
                          ->orderBy('asociado')
                          ->simplePaginate(1000);
        
        $grupoclientes = Asociado::whereIn('id', [3,2, 6])
                          ->where('asociado', 'LIKE', "%$nombreasociado%")
                          ->orderByRaw("FIELD(id, 6,2,3)")
                          ->simplePaginate(1000);
    ...
    public function anularcuentacobrar(Request $request)   
    {
        $registros = collect();

        if ($request->filled('search')) {
            $search = $request->search;
            $registros = Bateriasubcliente::where('id', 'like', "%$search%")->get();
        }

        $anulaciones = Bateriasubcliente::withTrashed() // Incluye los registros eliminados
            ->where('estado', 'ANULADO')
            ->whereNotNull('deleted_at') // Asegura que deleted_at tenga un valor
            ->orderBy('deleted_at', 'desc')
            ->get();

        return view('admin.asociados.anulaciones.anularcuentacobrar', compact('registros','anulaciones'));
    }
...
//CREAR Y EDITAR CLIENTE ITA
    public function crearclienteita(Asociado $asociado, Cliente $cliente)
    {
        $suc = [
        ...
        return view('admin.asociados.crearclienteita', compact('cliente','asociado', 'tipocliente', 'suc','tipoide', 'ciudadexp', 'estciv', 'genero', 'gradins', 'estlab', 'empresas', 'pais', 'ciudades', 'departamentos', 'aseguradoras', 'afp'));
    }
    public function pasaraclienteita(Request $request, Asociado $asociado, Cliente $cliente)
    {
        $suc = [
            'COCHABAMBA' => 'COCHABAMBA',
            'SANTA CRUZ' => 'SANTA CRUZ',
        ...
        $id = $request->query('precliente_id');

        if (!$id) {
            abort(404);
        }

        $precliente = DB::table('preclientes')->where('id', $id)->first();

        if (!$precliente) {
            abort(404);
        }

        return view('admin.asociados.pasaraclienteita', compact('cliente','asociado', 'tipocliente', 'suc','tipoide', 'ciudadexp', 'estciv', 'genero', 'gradins', 'estlab', 'empresas', 'pais', 'ciudades', 'departamentos', 'aseguradoras', 'afp','precliente'));
    }
    public function guardarclienteita(StoreClienteRequest $request)
    {
        $existeCliente = Cliente::where('ci', $request->ci)->exists();
        ...
        if ($request->paisnacimiento === 'BOLIVIA') {
            $clienteData['lugarnacimiento'] = Departamento::where('id', $request->lugarnacimiento_select)->value('departamento');
        } else {
            $clienteData['lugarnacimiento'] = $request->lugarnacimiento_text;
        }
                ...
                if ($userReferenciador) {
                    ...
                    if (!empty($userReferenciador->fcm_token)) {
                        FirebaseService::sendNotification(
                            ...
                            'Has ganado 20 GoodPoints por referenciar a ' . $cliente->nombrecompleto
                        );
                    }
                }
    ...
    public function guardarpreclienteita(StoreClienteRequest $request)
    {
        $existeCliente = Cliente::where('ci', $request->ci)->exists();
        ...
        // ✅ ACTUALIZAR ESTADO DEL PRECLIENTE A FINALIZADO
        if ($request->filled('idprecliente')) {
            PreCliente::where('id', $request->idprecliente)
    ...
    public function crearUsuario($clienteId)
    {
        ...
        $inicialesNombres = collect(explode(' ', trim($cliente->nombres)))
            ->map(fn($n) => strtoupper(substr($n, 0, 1)))
            ->implode('');

        $inicialPaterno = strtoupper(substr($cliente->apepaterno, 0, 1));
        $inicialMaterno = strtoupper(substr($cliente->apematerno, 0, 1));

        $passwordPlano = $inicialesNombres
                        . $inicialPaterno
                        . $inicialMaterno
                        . $cliente->ci;
    ...
    ->send(new CredencialesClienteMail($cliente, $passwordPlano));
        ...
        return back()->with(
            'info',
            'Usuario creado correctamente.'/*   /  '.'Correo: '. $cliente->email . '  /  Contraseña: ' . $passwordPlano */
        );
    }
    public function listadoclienteita(Request $request, Asociado $asociado) 
    {
        ...
        $clientes = Cliente::with('servicios')
                            ...
                            /* ->simplePaginate(20); */
                            ->get();
        $preclientes = PreCliente::where('nombrecompleto', 'LIKE', "%$nombrecompleto%")
                            ->where('estado', 'PENDIENTE')
                            ->orderBy('nombrecompleto')
                            /* ->simplePaginate(20); */
                            ->get();
        return view('admin.asociados.listadoclienteita', compact('asociado', 'clientes', 'preclientes'));
    }
    ...
    public function verclienteita(Cliente $cliente)
    {
        ...
        $requisitosubclientes = ProveedorInformefinal::where('clienteid', $cliente->id)->where('tipocliente', 'ITA')->get();
        $cartasclientes = CartasClientes::where('clienteid', $cliente->id)->get();
        ...
        $tieneContactos = Contactosubcliente::where('clienteid', $cliente->id)->where('tipocliente', 'ITA')->exists();
        ...
        $fechasbateriasSubCliente = BateriaSubCliente::where('clienteitaid', $IDCliente)
            ->distinct()
        ->pluck('fechabateria');
        $fechasRegistradas = ProveedorInformefinal::where('clienteid', $cliente->id)->where('tipocliente', 'ITA')
        ->pluck('fechabateria');
...
//ASIGNAR TRAMITE
    ...
    public function guardartramiteclienteita(StoreTramitesubclienteRequest $request)
    {
        ...
        $clienteData['clientenombre'] = $cliente->nombrecompleto;
        $clienteData['tipocliente'] = 'ITA';
...
//CREAR BATERIA CLIENTE ITA 
    public function crearbateriaclienteita(Cliente $cliente)
    {
        ...
        $departamentos = Departamento::orderBy('departamento')->pluck('departamento', 'id');
        $id = $cliente->nombrecompleto ? Cliente::where('id', $cliente->id)->value('id') : null;
        ...
        foreach ($fechasBateriaPorAccion as $accionid => $fecha) {
            $acciones = Bateriasubcliente::where('accionid', $accionid)
                ->where('clienteitaid', $idCliente)
                ->get(['id', 'precio', 'informe', 'proveedorasignado', 'fechabateria', 'fechainforme', 'usuarioregistro', 'orden', 'clienteitaid']);
            ...
            foreach ($acciones as $accion) {
                if ($accion->id && $accion->fechabateria) {
                    $accionesPorFecha[$accion->fechabateria][] = [
                        ...
                        'fechainforme' => $accion->fechainforme,
                        'usuarioregistro' => $accion->usuarioregistro,
                        'orden' => $accion->orden,
                        'clienteitaid' => $accion->clienteitaid,
                    ];
                }
            }
        } */
        ...
        foreach ($fechasBateriaPorAccion as $accionid => $fecha) {
            ...
            $fechainforme = Bateriasubcliente::where('accionid', $accionid)->where('clienteitaid', $idCliente)->value('fechainforme');
            $orden = Bateriasubcliente::where('accionid', $accionid)->where('clienteitaid', $idCliente)->value('orden');
            $usuarioregistro = Bateriasubcliente::where('accionid', $accionid)->where('clienteitaid', $idCliente)->value('usuarioregistro');
            $clienteitaid = Bateriasubcliente::where('accionid', $accionid)->where('clienteitaid', $idCliente)->value('clienteitaid');
    ...
    public function guardarBateria(Request $request)
    {
        foreach ($request->pacciondisponible as $idSubtramite => $accionSeleccionada) {
            ...
            BateriaSubCliente::create([
                ...
                'usuarioregistro' => auth()->user()->name,
                'clienteid' => $request->pclienteitaid[$idSubtramite],
                'clientenombre' => $request->pclienteitanombre[$idSubtramite],
                'tipocliente' => 'ITA',
            ]);
        }
    ...
    public function anular(Request $request) 
    {
        ...
        foreach ($registros as $registroData) {
            $id = $registroData['id'];
            $fechainforme = $registroData['fechainforme'];

            $registro = BateriaSubCliente::find($id);

            if ($registro) {
                $registro->motivoanulacion = 'MAL CARGADO';
                $registro->usuarioanulacion = $user->name;
                $registro->save();
                $registro->delete();
                $nuevoDatos = $registro->toArray();
                unset($nuevoDatos['id'], $nuevoDatos['created_at'], $nuevoDatos['updated_at'], $nuevoDatos['deleted_at']); 

                $nuevoDatos['precio'] = 0;
                $nuevoDatos['preciocompra'] = 0;
                $nuevoDatos['accionid'] = $registro->accionid . 'PA';
                $nuevoDatos['accionnombre'] = $registro->accionnombre . ' - PA';
                $nuevoDatos['servicio'] = 'AJENO';
                $nuevoDatos['pagoservicio'] = 'AJENO';
                $nuevoDatos['proveedorasignado'] = 'PROVEEDOR AJENO';
                $nuevoDatos['informe'] = 'SI TIENE INFORME';
                $nuevoDatos['motivoanulacion'] = null;
                $nuevoDatos['usuarioanulacion'] = null;
                $nuevoDatos['fechainforme'] = $fechainforme;
                
                $nuevoRegistro = BateriaSubCliente::create($nuevoDatos);

                $programacion = ProgramacionSubCliente::create(
                    ['proveedornombre' => $nuevoRegistro->proveedorasignado,
                    'fechabateria' => $nuevoRegistro->fechabateria,
                    'servicio' => 'AJENO',
                    'pagoservicio' => 'AJENO',
                    'clienteitaid' => $nuevoRegistro->clienteitaid,
                    'clienteitanombre' => $nuevoRegistro->clienteitanombre,
                    'fechaasignada' => $nuevoRegistro->fechabateria,
                    'horadesde' => now(),
                    'horahasta' => now(),
                    'horaasignada' => now(),
                    'areanombre' => $nuevoRegistro->areanombre,
                    'accionnombre' => $nuevoRegistro->accionnombre,
                    'precio' => '0',
                    'preciocompra' => '0',
                    'bateriaid' => $nuevoRegistro->id,
                    'usuarioid' => $nuevoRegistro->usuarioid,
                    'usuarioregistro' => $nuevoRegistro->usuarioregistro,
                    'clienteid' => $nuevoRegistro->clienteitaid,
                    'clientenombre' => $nuevoRegistro->clienteitanombre,
                    'tipocliente' => 'ITA']
                );
                EstadoProgramacionSubCliente::create(
                    ['fechabateria' => $nuevoRegistro->fechabateria,
                    'fechaatencionprogramacion' => $nuevoRegistro->fechabateria,
                    'clienteitaid' => $nuevoRegistro->clienteitaid,
                    'clienteitanombre' => $nuevoRegistro->clienteitanombre,
                    'accionnombre' => $nuevoRegistro->accionnombre,
    ...
    public function guardarbateriaclienteita(StoreBateriasubclienteRequest $request, Cliente $cliente)
    {
        ...
        if ($request->hasFile('orden')) {
            ...
            $carpetaCliente = public_path("/ordenesbateria/{$cliente->id}");
            if (!file_exists($carpetaCliente)) {
                mkdir($carpetaCliente, 0755, true);
            }
            $archivo_name = time() . '_' . $file->getClientOriginalName();
        ...
        $medicoderivante = $request->input('medicoderivante');
        $sucursalCliente = $cliente->sucursal;
            ...
            foreach ($areasSeleccionadas as $areaId) {
                ...
                foreach ($accionesSeleccionadas as $accionId) {
                    ...
                    if ($areaAccion) {
                        ...
                        $pagoservicio = $areaAccion->pagoservicio;
                        $comision = $areaAccion->comision;
                    } else {
                    ...
                    $clienteitaData['clienteid'] = $clienteID;
                    $clienteitaData['clientenombre'] = $cliente->nombrecompleto;
                    $clienteitaData['tipocliente'] = 'ITA';
                    $baseNombre = $accionNombre;
                    $baseId = $accionId;
                    $contador = 0;
                    do {
                        $sufijo = $contador === 0 ? '' : 'NV' . ($contador === 1 ? '' : $contador);
                        $nombreConSufijo = $baseNombre . ($sufijo ? ' - ' . $sufijo : '');
                        $idConSufijo = $baseId . ($sufijo ? $sufijo : '');
                    ...
                    if ($informe === 'SI TIENE INFORME') {
                        ...
                        do {
                            $sufijoPA = $contadorPA === 0 ? '' : ($contadorPA + 1);
...
//APROBAR COTIZACION DE PROGRAMACION DE CLIENTE ITA
    ...
    public function aprobarcotizacionprogramacionclienteita(Cliente $cliente) 
    {
        ...
        $cotizaciones = EstadoCotizacionSubCliente::where('clienteitaid', $cliente->id)
        ...
        $documentosPorFecha = EstadoCotizacionSubCliente::where('clienteitaid', $cliente->id)
            ...
            ->groupBy('fechabateria');
        $fecha = '';
        return view('admin.asociados.aprobarcotizacionprogramacionclienteita', compact('cotizaciones','fecha','documentosPorFecha','fechasregis','cliente', 'id', 'fechas', 'fechasRegistradas','fechasDisponibles'));
    } 
    ...
    public function guardaraprobacioncotizacionclienteita(StoreEstadocotizacionsubclienteRequest $request, Cliente $cliente)
    {
        ...
        $documentacioncotizacioncliente = Estadocotizacionsubcliente::create([
            ...
            'detalle' => 'COTIZACION Y CONSENTIMIENTO INFORMADO PARA LA REALIZACIÓN DE EVALUACIONES Y ESTUDIOS MÉDICOS ADICIONALES APROBADO',
        ]);
...
//CREAR PROGRAMACION Y REPROGRAMACION DE CLIENTE ITA
    public function crearprogramacionclienteita(Cliente $cliente, Request $request) 
    {
        ...
        $proveedoresAsignados = BateriaSubCliente::where('clienteitaid', $idCliente)
        ...
        $fechasBateriaPorAccion = BateriaSubCliente::where('clienteitaid', $idCliente)
        ...
        foreach ($proveedoresAsignados as $accion => $nombreProveedor) {
            ...
            if ($proveedor) {
                $proveedoresDetalles[$accion] = [
                    ...
                    'pagoservicio' => $proveedor->pagoservicio,
    ...
    public function guardarprogramacionclienteita(StoreProgramacionsubclienteRequest $request)
    {
        ...
        foreach ($accionesSeleccionadas as $accion) {
            ...
            $servicio = $request->input("servicio_$accionSanitizada");
            $pagoservicio = $request->input("pagoservicio_$accionSanitizada");
            ...
            if (!empty($idsubproc)) {
                ...
                if ($subproc) {
                    $subproc->update([
                        ...
                        'ordenprogramacion' => $ordenBateria,
                    ]);
                }
            }
    ...
    public function estadoprogramacionclienteita(Cliente $cliente, Request $request)
    {
        ...
        foreach ($accionesDisponibles as $accion) {
            ...
            if (/* $bateriaProveedor && $bateriaProveedor->servicio === 'EXTERNO' &&  */$proveedor) {
                ...
                $accion->ciudad = $proveedor->ciudad;
                $accion->ciudad2 = $proveedor->ciudad2;
                $accion->ciudad3 = $proveedor->ciudad3;
            } else {
    ...
    public function guardarestadoprogramacionclienteita(StoreEstadoprogramacionsubclienteRequest $request)  
    {
        ...
        //CODIGO TEMPORAL
            $cliente = Cliente::findOrFail($clienteitaid);
            $nombrecliente = $cliente->nombrecompleto;
        ...
        foreach ($accionesSeleccionadas as $accionNombre) {
            ...
            $accionId = Programacionsubcliente::where('accionnombre', $accionNombre)
                                            ->where('clienteitaid', $clienteitaid)
                                            ->where('fechabateria', $fechaBateria)
                                            ->value('id');
            ...
            if (in_array(strtoupper($accionNombre), ['LAVADO DE OIDO DERECHO', 'LAVADO DE OIDO IZQUIERDO'])) {
                ...
                $archivoDestino = $carpetaCliente . '/' . $archivo_name;

                if (file_exists($archivoOriginal)) {
                    copy($archivoOriginal, $archivoDestino);
                }
...
//CREAR DOCUMENTACION DE CLIENTE ITA
    public function creardocumentacionclienteita(Cliente $cliente, Asociado $asociado)
    {
        ...
        foreach ($accionesPorFecha as $fecha => $acciones) {
            foreach ($acciones as $accion) {
                ...
                $docfirmado = $registrado ? Documentacionsubcliente::where('accion', $accion->accionnombre)
                                                        ->where('fechabateria', $fecha)
                                                        ->where('clienteitaid', $IDcliente)
                                                        ->value('documentfirmado') : null;

                $docword = $registrado ? Documentacionsubcliente::where('accion', $accion->accionnombre)
                                                        ->where('fechabateria', $fecha)
                                                        ->where('clienteitaid', $IDcliente)
                                                        ->value('documentword') : null;
    ...
    public function guardardocumentacionclienteita(StoreDocumentacionsubclienteRequest $request, Cliente $cliente) 
    {
        ...
        foreach ($accionesSeleccionadas as $accionId) {
            ...
            $idprogramacion = Programacionsubcliente::where('fechabateria', $fechaBateria)
                                                       ->where('clienteitaid', $idcliente)
    ...
    public function guardarhistoriamedica(StoreDocumentacionsubclienteRequest $request, Cliente $cliente)  
    {
        ...
        if ($request->hasFile('archivo')) {
            ...
            // Buscar el siguiente número disponible para el archivo ZIP
            $baseZipName = 'HISTORIA_MEDICA_' . $cliente->nombrecompleto;
            $zipIndex = 1;
            $archivo_comprimido_name = $baseZipName . '.zip';
            $zip_path = $carpetaCliente . '/' . $archivo_comprimido_name;

            while (file_exists($zip_path)) {
                $zipIndex++;
                $archivo_comprimido_name = $baseZipName . "_{$zipIndex}.zip";
                $zip_path = $carpetaCliente . '/' . $archivo_comprimido_name;
            }
            ...
            // **Eliminar archivos previos en la carpeta de extracción**
            if (file_exists($extractPath)) {
                $archivosPrevios = glob($extractPath . '/*'); // Obtener todos los archivos dentro de la carpeta
                foreach ($archivosPrevios as $archivoPrevio) {
                    if (is_file($archivoPrevio)) {
                        unlink($archivoPrevio); // Eliminar archivo
                    }
                }
            } else {
        ...
        $documentacioncliente = Documentacionsubcliente::create(
            $request->except('accion') + [
                ...
                'document' => $archivoPDFDescomprimido,
                'accion' => $accion,
...
//CONTACTOS CLIENTE ITA
    public function vercontactoclienteita(Cliente $cliente)
    {
        ...
        $contactos = Contactosubcliente::where('clienteid', $idclienteita)
...
//DESCARGAR CHECK LIST PARA TODOS LOS TRAMITES
    ...
    public function descargarchecklistpensionmuerte(Request $request, Cliente $cliente)
    {
        $usuarioAutenticado = Auth::user();
        $documentosSeleccionados = json_decode($request->input('documentosSeleccionados'));
        $documentosSeleccionados2 = json_decode($request->input('documentosSeleccionados2'));
        ...
        $nombreDocumentos = [
            'poder' => 'PODER',
            'avcci' => 'AVC / CARNET ASEGURADO TITULAR (FALLECIDO)',
            'cnacasegurado' => 'CERTIFICADO NACIMIENTO ASEGURADO',
            'ciasegurado' => 'CARNET IDENTIDAD ASEGURADO',
            'cmatrimonio' => 'CERTIFICADO MATRIMONIO',
            'cnacconyuge' => 'CERTIFICADO NACIMIENTO CONYUGE (SOLICITANTE)',
            'ciconyuge' => 'CARNET IDENTIDAD CONYUGE (SOLICITANTE)',
            'cnacjihos' => 'CERTIFICADO NACIMIENTO HIJOS < 25',
            'cihijos' => 'CARNET IDENTIDAD HIJOS < 25',
            'crodomicilio' => 'CROQUIS DOMICILIO',
            'contrato' => 'CONTRATO',
            ...
            'ctrabajoinsalubre' => 'CERTIFICADO TRABAJO INSALUBRE',
            'poderciapoderado' => 'PODER Y CARNET IDENTIDAD APODERADO',
            'cmeddifuncion' => 'CERTIFICADO MÉDICO DE DEFUNCIÓN (ÓBITO)',
            'cnactitular' => 'CERTIFICADO NACIMIENTO TITULAR (FALLECIDO)',
            'cititular' => 'CARNET IDENTIDAD TITULAR (FALLECIDO)',
            'cestudioshijos' => 'CERTIFICADO ESTUDIOS HIJOS < 25',
            'tdeclaherederos' => 'TESTIMONIO DE DECLARATORIA DE HEREDEROS',
            'cnacherederos' => 'CERTIFICADO NACIMIENTO DECLARADOS HEREDEROS',
            'cideclaherederos' => 'CARNET IDENTIDAD DECLARADOS HEREDEROS',
        ...
        $pdf = PDF::loadView('admin.asociados.descargarchecklistpensionmuerte',compact('cliente', 'documentosSeleccionados',
                            'nombreDocumentos', 'documentosSeleccionados2', 'nombreDocumentos2','tramite'));
...
//REQUISITOS CLIENTE ITA TERCERA SOLICITUD
    ...
    {
        $tieneRequisitos = RequisitoSubCliente::where('clienteitaid', $cliente->id)
            ->where('servicio', 'TERCERA SOLICITUD')->exists();
        $estadoLaboral = strtolower($cliente->estadolaboral);
        $numHijosMenores = $cliente->numhijosmenores;
        $estadoCivil = strtolower($cliente->estadocivil);
        $servicio1 = strtolower($cliente->tipocliente);
...
//REQUISITOS CLIENTE ITA PENSIÓN POR MUERTE
    public function subirdocrequisitospensionmuerte(Cliente $cliente)
    {
        ...
        $campos2 = [
            'poder' => ['label' => 'PODER Y CARNET IDENTIDAD APODERADO', 'extra' => 'numeropoder'],
            'avcci' => ['label' => 'AVC/CARNET ASEGURADO TITULAR (FALLECIDO)'],
            'cnacasegurado' => ['label' => 'CERTIFICADO NACIMIENTO ASEGURADO'],
            'ciasegurado' => ['label' => 'CARNET IDENTIDAD ASEGURADO'],
            'cmatrimonio' => ['label' => 'CERTIFICADO DE MATRIMONIO'],
            'cnacconyuge' => ['label' => 'CERTIFICADO NACIMIENTO CONYUGE (SOLICITANTE)'],
            'ciconyuge' => ['label' => 'CARNET IDENTIDAD CONYUGE (SOLICITANTE)'],
            'cunionlibre' => ['label' => 'CERTIFICADO DE UNIÓN LIBRE'],
            'cnacimientounionlibre' => ['label' => 'CERTIFICADO DE NACIMIENTO DE UNIÓN LIBRE'],
            'ciunionlibre' => ['label' => 'CARNET IDENTIDAD DE UNIÓN LIBRE'],
            'cdivorcio' => ['label' => 'CERTIFICADO DE DIVORCIO'],
            ...
            'ctrabajoinsalubre' => ['label' => 'CERTIFICADO TRABAJO INSALUBRE'],
            'poderciapoderado' => ['label' => 'PODER Y CARNET IDENTIDAD APODERADO'],
            'cmeddifuncion' => ['label' => 'CERTIFICADO MÉDICO DE DEFUNCIÓN (ÓBITO)'],
            'cnactitular' => ['label' => 'CERTIFICADO NACIMIENTO TITULAR (FALLECIDO)'],
            'cititular' => ['label' => 'CARNET IDENTIDAD TITULAR (FALLECIDO)'],
            'cestudioshijos' => ['label' => 'CERTIFICADO ESTUDIOS HIJOS < 25'],
            'tdeclaherederos' => ['label' => 'TESTIMONIO DE DECLARATORIA DE HEREDEROS'],
            'cnacherederos' => ['label' => 'CERTIFICADO NACIMIENTO DECLARADOS HEREDEROS'],
            'cideclaherederos' => ['label' => 'CARNET IDENTIDAD DECLARADOS HEREDEROS'],
...
//DERIVAR A MEDICINA LABORAL
    public function generarPDFconsentimiento(Cliente $cliente, Request $request)
        {
            ...
            $proveedorAsignado = $request->input('proveedorasignado');
            ...
            $bateriaProveedor = BateriaProveedor::where('sucursal', $sucursalCliente)
                ...
                ->where('proveedor', $proveedorAsignado)
                ->first();
            ...
            $programacion = BateriaSubCliente::create([
                ...
                'pagoservicio' => $bateriaProveedor->pagoservicio, 
                'fechabateria' => now(),
                'usuarioid' => $usuarioId,
                'usuarioregistro' => $usuarioNombre,
                'clienteid' => $clienteitaId,
    ...
    public function aprobarinformefinaldirecto(Request $request, Cliente $cliente)
        {
            $nombres = $request->input('nombres');
            $apellidoPaterno = $request->input('apepaterno');
            $apellidoMaterno = $request->input('apematerno');
            ...
            $documentacion = Estadocotizacionsubcliente::create([
                'clienteitaid' => $clienteitaId,
                'clienteitanombre' => $nombreCompleto,
                'detalle' => 'APROBADO PARA INFORME FINAL DIRECTO',
                'usuarioid' => $usuarioId,
                'usuarioregistro' => $usuarioNombre,
                'document' => null,
                'tramite' => $tramite,
            ...
            Bateriasubcliente::create([
                ...
                'fechabateria' => now(),
                'accionid' => 'BA',
                'accionnombre' => 'BATERIA ARTIFICIAL',
                'clienteid' => $clienteitaId,
                'clientenombre' => $nombreCompleto,
                'tipocliente' => 'ITA',
            ]);
...
//PROVEEDOR INFORME FINAL CLIENTE ITA
    public function guardarproveedorinformefinal(StoreProveedorInformefinalRequest $request, Cliente $cliente)
    {
        ...
        $proveedorInformeFinal = ProveedorInformefinal::create([
            ...
            'atencionservicio' => 'INTERNO',
            'pagoservicio' => 'INTERNO',
        ...
        // Verificar si ya existe una batería con accionnombre = 'BATERIA ARTIFICIAL'
            $bateriaExistente = BateriaSubCliente::where('clienteid', $cliente->id)
            ->where('tipocliente', 'ITA')
            ->where('accionnombre', 'BATERIA ARTIFICIAL')
            ->first();

        if ($bateriaExistente) {
            // Actualizamos excepto la fecha
            $bateriaExistente->update([
                'clienteitanombre' => $cliente->nombrecompleto,
                'usuarioid' => $usuarioId,
                'usuarioregistro' => $usuarioRegistro,
                ...
                'pagoservicio' => 'INTERNO',
                'accionid' => 'IF',
                'provinfofinalid' => $proveedorInformeFinal->id,
            ...
            // No existe, así que creamos el registro nuevo
            BateriaSubCliente::create([
                'fechabateria' => $request->fechabateria,
                'clienteitaid' => $cliente->id,
                'clienteitanombre' => $cliente->nombrecompleto,
...
//
    public function guardarcartaclienteita(Cliente $cliente, Request $request) 
        {
            $archivo_name = null;
            if ($request->hasFile('carta')) {
                $file = $request->file('carta');
                $carpetaCliente = public_path("/cartasclientes/{$cliente->id}");
                if (!file_exists($carpetaCliente)) {
                    mkdir($carpetaCliente, 0755, true);
                }
                $archivo_name = time() . '_' . $file->getClientOriginalName();
...
//CREAR Y EDITAR CLIENTE COMUN
    ...
    {
        $existeCliente = ClienteComun::where('ci', $request->ci)->exists();
    ...
    public function guardarclientecomun(StoreClienteComunRequest $request) 
    {
        ...
        $ultimoRegistro = ClienteComun::orderBy('created_at', 'desc')->first();
        $ultimoId = $ultimoRegistro ? (int) filter_var($ultimoRegistro->id, FILTER_SANITIZE_NUMBER_INT) : 0;
        $nuevoId = ($ultimoId + 1) . 'C'; // Crear el nuevo ID con la "C" al final
        ...
        $clienteData = $request->all();
        $clienteData['ciudad'] = $ciudadNombre;
        $clienteData['id'] = $nuevoId; // Asignar el nuevo ID al cliente
    
        // Crear el cliente
        $clientecomun = ClienteComun::create($clienteData);
    
        // Redirigir con éxito
        return redirect()->route('admin.asociados.listadoclientecomun', 3)->with('info', 'El cliente se creó con éxito');
    }
    ...
    public function crearbateriaclientecomun(ClienteComun $clientecomun)
    {
        ...
        foreach ($areas as $id => $nombreArea) {
            $accionesPorArea[$id] = Bateriaproveedor::where('areasid', $id)
                ...
                ->orderBy('accion', 'asc')
                ->get(['id', 'accion', 'proveedor', 'precio', 'sesiones']);
        }
        ...
        $areas2 = Bateriaproveedor::orderBy('area', 'asc')
            ...
            ->where('sucursal', $sucursalCliente)
            ->where('asociado', 'CLIENTES COMUNES')
            ->get(['area', 'id', 'proveedor', 'precio' , 'sesiones']);
        ...
        foreach ($fechasBateriaPorAccion as $accionid => $fecha) {
            $acciones = Bateriasubcliente::where('accionid', $accionid)
                ->where('clientecomunid', $idCliente)
                ->get(['id', 'precio', 'informe', 'proveedorasignado', 'fechabateria', 'sesiones']);
        ...
        if ($tipoArea === 'Estudios') {
            ...
            foreach ($areasSeleccionadas as $areaId) {
                ...
                foreach ($accionesSeleccionadas as $accionId) {
                    ...
                    $clienteitaData['tipoarea'] = 'ESTUDIO';
                    $clienteitaData['clienteid'] = $clienteID;
                    $clienteitaData['clientenombre'] = $clientecomun->nombrecompleto;
                    $clienteitaData['tipocliente'] = 'COMUN';
                    ...
                    if ($informe === 'SI TIENE INFORME') {
                        ...
                        $clienteitaData['pagoservicio'] = $pagoservicio;
                        $clienteitaData['comision'] = $comision;
                        $clienteitaData['sesiones'] = $sesiones;
                    }
                    $clienteitaData['fechabateria'] = $fechaSeleccionada === 'nueva_bateria' ? $fechaActual : $fechaSeleccionada;
                    $clienteitaData['informe'] = $informe;
                    $clienteitaData['usuarioid'] = $usuarioID;
...
//CREAR PROGRAMACION Y REPROGRAMACION DE CLIENTE COMUN
    public function crearprogramacionclientecomun(ClienteComun $clientecomun)
    {
        ...
        foreach ($fechasBateria as $fecha) {
            $accionesProgramadas = ProgramacionSubCliente::where('fechabateria', $fecha)
                ->where('clientecomunid', $clientecomunid)
                ->get(['id', 'accionnombre','proveedornombre', 'fechaasignada', 'horadesde', 'horahasta', 'precio', 'preciocompra', 'servicio', 'pagoservicio', 'bateriaid', 'nrosesion']);

            foreach ($accionesProgramadas as $accion) {
                // Aseguramos que estamos usando un array para cada 'accionnombre'
                $accionesDetallesPorFecha[$fecha][$accion->accionnombre][] = $accion;
            }
        }
    ...
    public function actualizarFecha(Request $request, $id)
    {
        ...
        if ($registro) {
            $registro->fechaasignada = $request->fechaasignada;
            $registro->horadesde = $request->horadesde;
            $registro->horahasta = $request->horahasta;
            $registro->save();
    ...
    public function guardarprogramacionclientecomun(StoreProgramacionsubclienteRequest $request) 
    {
        ...
        foreach ($accionesSeleccionadas as $accion) {
            ...
            $proveedornombre = $request->input("proveedor_$accionSanitizada");
            $areanombre = $request->input("areanombre_$accionSanitizada");
            $precioTotal = (float) $request->input("precio_$accionSanitizada");
            $precioCompraTotal = (float) $request->input("preciocompra_$accionSanitizada");
            $servicio = $request->input("servicio_$accionSanitizada");
            $pagoservicio = $request->input("pagoservicio_$accionSanitizada");
            $bateriaid = $request->input("bateriaid_$accionSanitizada");
            $sesiones = (int) ($request->input("sesiones_$accionSanitizada") ?? 1);
        
            $precioPorSesion = number_format($precioTotal / $sesiones, 2, '.', '');
            $precioCompraPorSesion = number_format($precioCompraTotal / $sesiones, 2, '.', '');
        
            for ($i = 1; $i <= $sesiones; $i++) {
                $nrosesion = ($sesiones > 1) ? "SESIÓN $i/$sesiones" : null;
                $fechaSesion = ($i == 1) ? $fechaasignada : null;
                $horaSesion = ($i == 1) ? $horaasignada : null;
                $horadesdeSesion = ($i == 1) ? $horadesde : null;
                $horahastaSesion = ($i == 1) ? $horahasta : null;
        
                $query = Programacionsubcliente::where('accionnombre', $accion)
                    ->where('fechabateria', $fechabateria)
                    ->where('clientecomunid', $clientecomunid);
        
                if ($nrosesion) {
                    $query->where('nrosesion', $nrosesion);
                }
        
                $existente = $query->exists();
        
                if (!$existente) {
                    Programacionsubcliente::create([
                        'accionnombre' => $accion,
                        'nrosesion' => $nrosesion,
                        'horaasignada' => $horaSesion,
                        'fechaasignada' => $fechaSesion,
                        'proveedornombre' => $proveedornombre,
                        'clientecomunid' => $clientecomunid,
                        'clientecomunnombre' => $clientecomunnombre,
                        'horadesde' => $horadesdeSesion,
                        'horahasta' => $horahastaSesion,
                        'fechabateria' => $fechabateria,
                        'areanombre' => $areanombre,
                        'precio' => $precioPorSesion,
                        'preciocompra' => $precioCompraPorSesion,
                        'servicio' => $servicio,
                        'pagoservicio' => $pagoservicio,
                        'usuarioid' => Auth::id(),
                        'usuarioregistro' => Auth::user()->name,
                        ...
                        'clientenombre' => $clientecomunnombre,
                        'tipocliente' => 'COMUN'
                    ]);
                }
            }
        ...
        return redirect()->route('admin.asociados.crearprogramacionclientecomun', $request->clientecomun)
            ->with('info', 'La programación del cliente se creó con éxito');
    }
    public function reprogramacionclientecomun(ClienteComun $clientecomun, Request $request)
    {
        ...
        return view('admin.asociados.reprogramacionclientecomun', compact('proveedorprogramacion','reprogramaciones','programacionsubclientes', 'id', 'clientecomun', 'fechas', 'total', 'fechaSeleccionada'));
    }
    ...
    public function guardarreprogramacionclientecomun(Request $request, Programacionsubcliente $programacionsubcliente)
    {
        ...
        $cliente = ClienteComun::where('nombrecompleto', $programacionsubcliente->clientecomunnombre)->first();
        $fechaSeleccionada = $request->input('buscarpor');

        return redirect()->route('admin.asociados.reprogramacionclientecomun', ['clientecomun' => $cliente, 'buscarpor' => $fechaSeleccionada])
            ->with('eliminar', 'ok');
    }
    ...
    public function guardarestadoprogramacionclientecomun(StoreEstadoprogramacionsubclienteRequest $request)
    {
        ...
        $cliente = ClienteComun::findOrFail($clientecomunid);
        $nombrecliente = $cliente->nombrecompleto;

        foreach ($accionesSeleccionadas as $accionNombre) {
            
        $acciones = Programacionsubcliente::where('accionnombre', $accionNombre)
                                            ->where('clientecomunid', $clientecomunid)
                                            ->where('fechabateria', $fechaBateria)
                                            ->get();
            
            foreach ($acciones as $accion) {
                $registroExistente = Estadoprogramacionsubcliente::where('programacionid', $accion->id)->exists();
...
//CREAR Y EDITAR CLIENTE AUDITORIA
    ...
    public function verclienteauditoriamomentaneo(ClienteAuditoria $clienteauditoria)
    {
        ...
        $requisitosubclientes = ProveedorInformefinal::where('clienteid', $clienteauditoria->id)->where('tipocliente', 'AUDITORIA')->get();
        ...
        $proveedores = Proveedor::whereIn('id', [3, 54])->get(['id', 'proveedor', 'celular']);
        $tieneContactos = Contactosubcliente::where('clienteid', $clienteauditoria->id)->exists();
        ...
        $fechasRegistradas = ProveedorInformefinal::where('clienteid', $clienteauditoria->id)
            ->where('tipocliente', 'AUDITORIA')
            ->pluck('fechabateria');
    ...
    public function verclienteauditoria(ClienteAuditoria $clienteauditoria)
    {
        ...
        $informefinalau = Informefinal::where('clienteauditoriaid', $clienteauditoria->id)->get();
        $proveedores = Proveedor::whereIn('id', [3, 54])->get(['id', 'proveedor', 'celular']);
        $tieneContactos = Contactosubcliente::where('clienteid', $clienteauditoria->id)->where('tipocliente', 'AUDITORIA')->exists();
        $tieneTramites = Tramitesubcliente::where('clienteauditoriaid', $clienteauditoria->id)->exists();
        $tieneBateria = Bateriasubcliente::where('clienteauditoriaid', $clienteauditoria->id)->exists();
        $tieneProgramacion = Programacionsubcliente::where('clienteauditoriaid', $clienteauditoria->id)->exists();
        $tieneCotizacionaprobada = Estadocotizacionsubcliente::where('clienteauditoriaid', $clienteauditoria->id)->exists();
...
//ASIGNAR TRAMITE
    ...
    public function guardartramiteclienteauditoria(StoreTramitesubclienteRequest $request)
    {
        ...
        $clienteData['estado'] = $request->input('estado');
        $clienteData['clienteid'] = $clienteID;
        $clienteData['clientenombre'] = $clienteauditoria->nombrecompleto;
        $clienteData['tipocliente'] = 'AUDITORIA';
        $tramitesubcliente = Tramitesubcliente::create($clienteData);
...
//CREAR BATERIA CLIENTE AUDITORIA
    public function crearbateriaclienteauditoria(ClienteAuditoria $clienteauditoria)
    {
        ...
        if (in_array($nombreusuario, $usuariosConPermisoSiempre)) {
            ...
            if ($permisoCodigo) {
                if ($permisoCodigo->permisoSolicitado === 'admin.asociados.crearbateriaclienteauditoria') {
                    $horaActivacion = Carbon::parse($permisoCodigo->horaActivacion);
                    $tiempoLimite = $permisoCodigo->tiempoLimite;
                    $horaExpiracion = $horaActivacion->copy()->addMinutes($tiempoLimite);
        ...
        $departamentos = Departamento::orderBy('departamento')->pluck('departamento', 'id');
        $id = $clienteauditoria->nombrecompleto ? ClienteAuditoria::where('id', $clienteauditoria->id)->value('id') : null;
        ...
        foreach ($fechasBateriaPorAccion as $accionid => $fecha) {
            ...
            $fechainforme = Bateriasubcliente::where('accionid', $accionid)->where('clienteauditoriaid', $idCliente)->value('fechainforme');
            $orden = Bateriasubcliente::where('accionid', $accionid)->where('clienteauditoriaid', $idCliente)->value('orden');
            $usuarioregistro = Bateriasubcliente::where('accionid', $accionid)->where('clienteauditoriaid', $idCliente)->value('usuarioregistro');
            $clienteauditoriaid = Bateriasubcliente::where('accionid', $accionid)->where('clienteauditoriaid', $idCliente)->value('clienteauditoriaid');
        ...
        $proveedoresmedicos = Proveedor::all(); 
        return view('admin.asociados.crearbateriaclienteauditoria', compact('proveedoresmedicos','permisoValido', 'fechaExpiracion',
            'accionesPorFecha',
            ...
            'id',
            'accionesCliente',
            'areas2','rolusuario','nombreusuario'
        ));
    }
    public function anularbateriaauditoria(Request $request) 
    {
        $registros = $request->input('registros');
        $user = auth()->user();
        ...
        foreach ($registros as $registroData) {
            ...
            if ($registro) {
                ...
                $programacion = ProgramacionSubCliente::create(
                    ...
                    'servicio' => 'AJENO',
                    'pagoservicio' => 'AJENO',
                    'clienteauditoriaid' => $nuevoRegistro->clienteauditoriaid,
                    'clienteauditorianombre' => $nuevoRegistro->clienteauditorianombre,
                    'fechaasignada' => $nuevoRegistro->fechabateria,
                    'horadesde' => now(),
                    'horahasta' => now(),
                    'horaasignada' => now(),
                    ...
                    'usuarioid' => $nuevoRegistro->usuarioid,
                    'usuarioregistro' => $nuevoRegistro->usuarioregistro,
                    'clienteid' => $nuevoRegistro->clienteauditoriaid,
                    'clientenombre' => $nuevoRegistro->clienteauditorianombre,
    ...
    public function guardarbateriaclienteauditoria(StoreBateriasubclienteRequest $request, ClienteAuditoria $clienteauditoria)
    {
        if ($request->filled('codigo')) {
            ...
            if ($permisoCodigo) {
                if ($permisoCodigo->permisoSolicitado === 'admin.asociados.crearbateriaclienteauditoria') {
                    ...
                    $permisoCodigo->estado = 'activo';
                    $permisoCodigo->save();
                    return redirect()->route('admin.asociados.crearbateriaclienteauditoria', ['clienteauditoria' => $clienteauditoria->id])
                        ->with('success', 'Código ingresado correctamente.');
                } else {
                    $permisoCodigo->estado = 'expirado';
                    $permisoCodigo->save();
        ...
        $archivo_name = null;
            if ($request->hasFile('orden')) {
                $file = $request->file('orden');
                $carpetaCliente = public_path("/ordenesbateria/{$clienteauditoria->id}");
                if (!file_exists($carpetaCliente)) {
                    mkdir($carpetaCliente, 0755, true);
                }
                $archivo_name = time() . '_' . $file->getClientOriginalName();
        ...
        if ($tipoArea === 'Estudios') {
            ...
            foreach ($areasSeleccionadas as $areaId) {
                ...
                foreach ($accionesSeleccionadas as $accionId) {
                    ...
                    $clienteitaData['tipoarea'] = 'ESTUDIO';
                    $clienteitaData['clienteid'] = $clienteID;
                    $clienteitaData['clientenombre'] = $clienteauditoria->nombrecompleto;
                    $clienteitaData['tipocliente'] = 'AUDITORIA';
    ...
    public function duplicar(Request $request)
    {
        try {
            ...
            if (empty($registros) || empty($clienteAuditoriaID)) {
            ...
            foreach ($registros as $id) {
                $registro = BateriaSubCliente::find($id);
                if ($registro) {
                    $nuevoRegistro = BateriaSubCliente::create([
                        'fechabateria' => now(),
                        'accionnombre' => $registro->accionnombre,
                        'proveedorasignado' => $registro->proveedorasignado,
                        'precio' => $registro->precio,
                        'informe' => $registro->informe,
                        'usuarioid' => $registro->usuarioid,
                        'usuarioregistro' => $registro->usuarioregistro,
                        'clienteitaid' => null, 
                        'clienteitanombre' => null, 
                        'clienteauditoriaid' => $clienteAuditoriaID,
                        'clienteauditorianombre' => $registro->clienteitanombre,
                        ...
                        'fechainforme' => $registro->fechainforme,
                        'antecedentes' => $registro->antecedentes,
                        'pagoservicio' => $registro->pagoservicio,
                        'comision' => $registro->comision,
                        'sesiones' => $registro->sesiones,
                        'pagoatencion' => $registro->pagoatencion,
                        'medicoderivante' => $registro->medicoderivante,
                        'clienteid' => $clienteAuditoriaID,
                        'clientenombre' => $registro->clienteitanombre,
                        'tipocliente' => 'AUDITORIA',
                    ]);

                    if ($registro->proveedorasignado === 'PROVEEDOR AJENO') {
                        $programacion = ProgramacionSubCliente::create([
                            'proveedornombre' => $nuevoRegistro->proveedorasignado,
                            'fechabateria' => now(),
                            'servicio' => $nuevoRegistro->servicio,
                            'pagoservicio' => $nuevoRegistro->pagoservicio,
                            'clienteauditoriaid' => $nuevoRegistro->clienteauditoriaid,
                            'clienteauditorianombre' => $nuevoRegistro->clienteauditorianombre,
                            'fechaasignada' => $nuevoRegistro->fechabateria,
                            'horadesde' => now(),
                            ...
                            'areanombre' => $nuevoRegistro->areanombre,
                            'accionnombre' => $nuevoRegistro->accionnombre,
                            'precio' => $nuevoRegistro->precio,
                            'preciocompra' => $nuevoRegistro->preciocompra,
                            'usuarioid' => $nuevoRegistro->usuarioid,
                            'usuarioregistro' => $nuevoRegistro->usuarioregistro,
                            'bateriaid' => $nuevoRegistro->id,
                            'comision' => $nuevoRegistro->comision,
                            'medicoderivante' => $nuevoRegistro->medicoderivante,
                            'clienteid' => $nuevoRegistro->clienteauditoriaid,
                            'clientenombre' => $nuevoRegistro->clienteauditorianombre,
                            'tipocliente' => 'AUDITORIA',
                        ]);
                        ...
                        EstadoProgramacionSubCliente::create([
                            'clienteauditoriaid' => $programacion->clienteauditoriaid,
                            'clienteauditorianombre' => $programacion->clienteauditorianombre,
                            'fechaatencionprogramacion' => now(),
                            'usuarioid' => $programacion->usuarioid,
                            'usuarioregistro' => $programacion->usuarioregistro,
                            'accionnombre' => $programacion->accionnombre,
                            'fechabateria' => now(),
                            'programacionid' => $programacion->id,
                            'clienteid' => $programacion->clienteauditoriaid,
                            'clientenombre' => $programacion->clienteauditorianombre,
                            'tipocliente' => 'AUDITORIA',
                        ]);
                    }
            ...
            \Log::error('Error en duplicación: ' . $e->getMessage());
...
//CREAR PROGRAMACION Y REPROGRAMACION DE CLIENTE AUDITORIA
    public function crearprogramacionclienteauditoria(ClienteAuditoria $clienteauditoria, Request $request)
    {
        ...
        $proveedoresAsignados = BateriaSubCliente::where('clienteauditoriaid', $idCliente)
            ->whereIn('accionnombre', $accionesCliente)
            ->pluck('proveedorasignado', 'accionnombre')
            ->toArray();

        $fechasEnEstadoCotizacionSubCliente = EstadoCotizacionSubCliente::where('clienteauditoriaid', $idCliente)  
            ->distinct()
            ->pluck('fechabateria');
    
        $fechasBateriaPorAccion = BateriaSubCliente::where('clienteauditoriaid', $idCliente)
            ->where(function ($query) use ($fechasEnEstadoCotizacionSubCliente) {
                $query->whereIn('fechabateria', $fechasEnEstadoCotizacionSubCliente)
                    ->orWhere('accionnombre', 'MEDICINA LABORAL');
            })
    ...
    public function guardarreprogramacionclienteauditoria(Request $request, Programacionsubcliente $programacionsubcliente)
    {
        ...
        return redirect()->route('admin.asociados.reprogramacionclienteauditoria', ['clienteauditoria' => $clienteauditoria, 'buscarpor' => $fechaSeleccionada])
            ->with('eliminar', 'ok');
    }
    ...
    public function guardarestadoprogramacionclienteauditoria(StoreEstadoprogramacionsubclienteRequest $request)
    {
        ...
        //CODIGO TEMPORAL
            $clienteauditoria = ClienteAuditoria::findOrFail($clienteauditoriaid);
            $nombrecliente = $clienteauditoria->nombrecompleto;
...
//CREAR DOCUMENTACION DE CLIENTE AUDITORIA
    public function creardocumentacionclienteauditoria(ClienteAuditoria $clienteauditoria, Asociado $asociado)
    {
        ...
        $accionesNoRegistradasPorFecha = Estadoprogramacionsubcliente::join('programacionsubclientes', function ($join) {
                $join->on('estadoprogramacionsubclientes.accionnombre', '=', 'programacionsubclientes.accionnombre')
                    ->on('estadoprogramacionsubclientes.clienteauditoriaid', '=', 'programacionsubclientes.clienteauditoriaid')
                    ->on('estadoprogramacionsubclientes.fechabateria', '=', 'programacionsubclientes.fechabateria');
            })
            ->where('estadoprogramacionsubclientes.clienteauditoriaid', $IDcliente)
            ->whereNull('estadoprogramacionsubclientes.deleted_at')
            ->whereNull('programacionsubclientes.deleted_at')
            ->when($usuario->hasRole('PROVEEDOR'), function ($query) use ($usuario) {
    ...
    public function guardarhistoriamedicaauditoria(StoreDocumentacionsubclienteRequest $request, ClienteAuditoria $clienteauditoria) 
    {
        ...
        if ($request->hasFile('archivo')) {
            ...
            // Buscar el siguiente número disponible para el archivo ZIP
            $baseZipName = 'HISTORIA_MEDICA_' . $clienteauditoria->nombrecompleto;
            $zipIndex = 1;
            $archivo_comprimido_name = $baseZipName . '.zip';
            $zip_path = $carpetaCliente . '/' . $archivo_comprimido_name;
...
//CONTACTOS CLIENTE AUDITORIA
    public function vercontactoclienteauditoria(ClienteAuditoria $clienteauditoria)
    {
        ...
        $contactos = Contactosubcliente::where('clienteid', $idclienteauditoria)
                                ->where('tipocliente', 'AUDITORIA')
                                ->get();
    ...
    public function generarchecklistclienteauditoria(ClienteAuditoria $clienteauditoria)
        {
            ...
            $registroaprobadoExistente = Estadocotizacionsubcliente::where('clienteauditoriaid', $clienteauditoria->id)
                ->where('detalle', 'APROBADO PARA INICIAR A CREAR BATERIA')
                ->first();
            $registroaprobadoinformefinalExistente = Estadocotizacionsubcliente::where('clienteauditoriaid', $clienteauditoria->id)
                ->where('detalle', 'APROBADO PARA INFORME FINAL DIRECTO')
                ->first();
                $userSucursal = Auth::user()->sucursal;
...
//
//PROVEEDOR INFORME FINAL CLIENTE AUDITORIA
    public function guardarproveedorinformefinalauditoria(StoreProveedorInformefinalRequest $request, ClienteAuditoria $clienteauditoria)
    {
        ...
        // Verificar si ya existe una batería con accionnombre = 'BATERIA ARTIFICIAL'
            $bateriaExistente = BateriaSubCliente::where('clienteauditoriaid', $clienteauditoria->id)
            ->where('accionnombre', 'BATERIA ARTIFICIAL')
            ->first();
...
//
//DICTAMEN
    public function guardardictamenauditoria(Request $request, ClienteAuditoria $clienteauditoria)
    {
        $archivo_name = null;
        ...
        if ($request->hasFile('documento')) {
            ...
            $carpetaCliente = public_path("/dictamenauditoria/{$clienteauditoria->id}");
            if (!file_exists($carpetaCliente)) {
                mkdir($carpetaCliente, 0755, true);}
            $archivo_name = time() . '_' . $file->getClientOriginalName();
            $file->move($carpetaCliente, $archivo_name);
        ...
        return redirect()->route('admin.asociados.verclienteauditoria', $clienteauditoria)
            ->with('info', 'Dictamen guardado exitosamente.');
    }
//
//CARTAS ACTIVACION DESGRAVAMEN

    public function cartasactdesgravamen(Request $request, ClienteAuditoria $clienteauditoria)
    {
        $bancos = DB::table('clienteauditorias')
        ->where('id', $clienteauditoria->id)
        ->select('banco1', 'banco2', 'banco3', 'nrocredito1', 'nrocredito2', 'nrocredito3', 'nrocredito4', 'nrocredito5', 'nrocredito6', 'nrocredito7', 'nrocredito8', 'nrocredito9', 'nrocredito10', 'nrocredito11', 'nrocredito12', 'nrocredito13', 'nrocredito14', 'nrocredito15', 'nrocredito16', 'nrocredito17', 'nrocredito18')
        ->first();

        $opciones = $bancos;

        $dictamen = DB::table('dictamenauditoria')
            ->where('clienteauditoriaid', $clienteauditoria->id)
            ->first(['nrodictamen', 'fechadictamen', 'porcentajeinvalidez']);

        // Verifica si el registro existe
        $nrodictamen = $dictamen ? ($dictamen->nrodictamen ?: 'Vacío') : 'Vacío';
        $fechadictamen = $dictamen ? ($dictamen->fechadictamen ?: 'Vacío') : 'Vacío';
        $porcentajeinvalidez = $dictamen ? ($dictamen->porcentajeinvalidez ?: 'Vacío') : 'Vacío';
        
        if (!$dictamen) {
            $dictamen = (object)[
                'nrodictamen' => 'NO DISPONIBLE',
                'fechadictamen' => 'NO DISPONIBLE',
                'porcentajeinvalidez' => 'NO DISPONIBLE'
            ];
        }
        
        $acciones = DB::table('bateriasubclientes')
        ->where('clienteauditoriaid', $clienteauditoria->id)
        ->get(['accionnombre', 'tipoarea']);

        return view('admin.asociados.cartasdesgravamen.cartasactdesgravamen', [
            'opciones' => $opciones,
            'clienteauditoria' => $clienteauditoria,
            'dictamen' => $dictamen,
            'acciones' => $acciones,
            'nrodictamen' => $nrodictamen,
            'fechadictamen' => $fechadictamen,
            'porcentajeinvalidez' => $porcentajeinvalidez
        ]);
    }
    public function descargarcartaactdesgravamen(Request $request, ClienteAuditoria $clienteauditoria)
    {
        $clienteauditoriaid = $request->input('clienteauditoriaid');
        $clienteauditorianombre = $request->input('clienteauditorianombre');
        $ci = $request->input('ci');
        $dictamen = DictamenAuditoria::where('clienteauditoriaid', $clienteauditoriaid)->first();
        $fecha = $request->input('fecha');
        ...
        $documentosSeleccionados = json_decode($request->input('documentos_seleccionados'));
        $accionesSeleccionadas = json_decode($request->input('acciones_seleccionadas'));
        $clienteAuditoria = ClienteAuditoria::find($clienteauditoriaid);

        if (!$clienteAuditoria) {
            return response()->json(['error' => 'Cliente no encontrado'], 404);
        }

        // Obtener el valor del banco según la opción seleccionada
        switch ($opcion) {
            case 'banco1':
                $nombreBanco = $clienteAuditoria->banco1; // Asume que tienes un campo banco1
                break;
            case 'banco2':
                $nombreBanco = $clienteAuditoria->banco2; // Asume que tienes un campo banco2
                break;
            case 'banco3':
                $nombreBanco = $clienteAuditoria->banco3; // Asume que tienes un campo banco3
                break;
            default:
                $nombreBanco = 'Banco Desconocido'; // En caso de que la opción no sea válida
                break;
        }
        ...
        $nuevaCarta->nombrecarta = 'ACTIVACION DE SEGURO DE DESGRAVAMEN';
        $nuevaCarta->usuarioregistroid = $usuario->id;
        $nuevaCarta->usuarioregistronombre = $usuario->name;

        $html = View::make('admin.asociados.cartasdesgravamen.activacionsegdesgravamen', compact(
            'fecha', 'ciudad', 'opcion', 'clienteuno', 'clienteunoci', 'clientedos', 'clientedosci'
            ,'dictamen','clienteauditorianombre','ci','nombreBanco' ,'creditos','documentosSeleccionados','accionesSeleccionadas'
        ))->render();
        ...
        $fileName = $dateTime . 'Activacio-Seguro-Desgravamen-' . str_replace(' ', '_', $clienteuno) . '-' . str_replace(' ', '_', $clientedos) . '.pdf';
    ...
    public function descargarcartaactcoberturadesgravamen(Request $request, ClienteAuditoria $clienteauditoria)
    {
        $clienteauditoriaid = $request->input('clienteauditoriaid');
        $clienteauditorianombre = $request->input('clienteauditorianombre');
        $ci = $request->input('ci');
        ...
        // Recibir los artículos seleccionados
        $articulosSeleccionados = explode('|', $request->input('articulosSeleccionados'));

        // Procesar los artículos
        $articulosTexto = implode('<br>', $articulosSeleccionados);
            ...
            $usuario = Auth::user();
            $nuevaCarta = new CartasPolizas();
            $ref_carta = $request->input('ref_carta');
            $ref_carta2 = strip_tags($ref_carta);
            $ref_carta3 = str_replace('REF: ', '', $ref_carta2);
            $nuevaCarta->fecha = $fecha;
            $nuevaCarta->ciudad = $ciudad;
            $nuevaCarta->banco = $nombreBanco;
            $nuevaCarta->nombreclienteuno = $clienteuno;
            ...
            $nuevaCarta->nombreclientedos = $clientedos;
            $nuevaCarta->ciclientedos = $clientedosci;
            $nuevaCarta->nombrecarta = $ref_carta3;
            $nuevaCarta->usuarioregistroid = $usuario->id;
            $nuevaCarta->usuarioregistronombre = $usuario->name;
            $ref_carta = $request->input('ref_carta');
            $articulos = $request->input('articulos');

            $html = View::make('admin.asociados.cartasdesgravamen.activacioncoberturadesgravamen', compact(
                'fecha', 'ciudad', 'opcion', 'clienteuno', 'clienteunoci', 'clientedos', 'clientedosci'
                ,'dictamen','clienteauditorianombre','ci','nombreBanco', 'articulosTexto', 'ref_carta'
            ))->render();
...
//CREAR PROGRAMACION Y REPROGRAMACION DE CLIENTE COMUN
    public function crearprogramacionclientebanco(ClienteBanco $clientebanco)
    {
        ...
        foreach ($fechasBateria as $fecha) {
            $accionesProgramadas = ProgramacionSubCliente::where('fechabateria', $fecha)
                ->where('clientebancoid', $idCliente)
                ->get(['id', 'accionnombre','proveedornombre', 'fechaasignada', 'horadesde', 'horahasta', 'horahasta', 'precio', 'preciocompra', 'servicio', 'pagoservicio']);
...
//
    ...
    public function crearempresa(Request $request)
    {
        ...
        $empresa = Empresa::create([
            ...
            'telefono' => $request->input('telefono'), // Asegúrate de agregar el teléfono si es necesario
            'direccion' => $request->input('direccion'), // Agregar dirección si es necesario
        ]);

        // Retornar una respuesta JSON con los datos de la empresa
        return response()->json([
            'success' => true,
            'empresa' => $empresa, // Devolver la empresa recién creada
        ]);
    }

    public function obtenerEmpresas()
    {
        // Obtener todas las empresas ordenadas alfabéticamente
        $empresas = Empresa::orderBy('nombreempresa', 'asc')->get();
